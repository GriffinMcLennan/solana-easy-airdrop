...
import { TOKEN_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID, AccountLayout, getAssociatedTokenAddressSync } from "@solana/spl-token";
...
async function getUserClaimAndProof(rootHex: string, address: string) {
    const response = await fetch(`http://localhost:5000/api/airdrop/${rootHex}/${address}`);
...
export function useClaimAirdrop() {
    ...
    const claimAirdrop = useCallback(async (merkleRootHash: number[]) => {
        if (!program || !wallet) {
       ...
       const merkleRoot = PublicKey.findProgramAddressSync([Buffer.from("merkle_root"), Buffer.from(merkleRootHash)], program.programId)[0];

       // Find token accounts owned by the merkle root to determine the mint
       const tokenAccounts = await connection.getTokenAccountsByOwner(merkleRoot, {
       ...
       const claimIx = await program.methods.claim(proof, new BN(claim.amount), claim.leaf_index).accounts({
        merkleRoot,
        mint,
